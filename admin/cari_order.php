...
if ( ($_GET['act']=="delete") && !empty($_GET[id]) ){
    @mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM php_shop_orders WHERE id='".$_GET[id]."'");
    echo"data terhapus";
}
...
echo"<table id='data_pembeli'>";
    ...
    $status="<q>Baru</q>";
           ...
           echo"<table id='alamat_order'>";
            ...
            $orders_info=@mysqli_result(@mysqli_query($GLOBALS["___mysqli_ston"], "SELECT orders_info FROM php_shop_orders WHERE kode_order='".$_GET['id']."'"), 0, 0);
            echo '
            <style>
